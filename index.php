 ...
 * @autor Miguel �ngel Garc�a Gonz�lez <miguel@iesmonterroso.org>
 * @copyright Miguel �ngel Garc�a Gonz�lez, <miguel@iesmonterroso.org>, http://esmonterroso.org/intranet/
 * @licencia http://www.gnu.org/licenses/gpl.html GNU GPL
 * @paquete Intranet del IES Monterroso, Consejer�a de Educaci�n de la junta de Andalucia.
...
// Arhivo de configuraci�n cargado
...
hacerlo para entrar en la Intranet.<br />Vuelve atr�s e int�ntalo de nuevo.          
			</div>
          </div> 
          <br />
 ...
 <?
	...
	// O no ha escrito el usuario o bien est� intentando entrar ilegalmente
	if (empty ( $codigo )) {
		echo $cabecera;
				?>
    <br />
...
Vuelve atr�s e int�ntalo de nuevo.         
			</div>
          </div> 
...
correcta.Vuelve atr�s e int�ntalo de nuevo. Y no olvides que hay que respetar la
diferencia entre may�sculas y min�sculas.        
			</div>
          </div> 
          <br /><form><input name="volver" type="button" class="btn btn-primary" onClick="history.go(-1)"
	value="Volver"></form>   
...
<br />
    <div align="justify"><div class="alert alert-danger alert-block fade in" style="max-width:360px;">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
			<h4 class="lead">ATENCI�N:</h4>Parece que tenemos un problema con el archivo de configuraci�n de la aplicaci�n. 
			No se puede escribir en el archivo, y eso indica que hay problemas. Debes asegurarte que el directorio donde has
			colocado l�os archivos de la aplicaci�n tiene permiso de escritura. De lo contrario, no podremos continuar...
			</div>
          </div> 
<?
}
...
<br />
    <div align="justify"><div class="alert alert-danger alert-block fade in" style="max-width:360px;">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
			<h4 class="lead">ATENCI�N:</h4>Parece que tenemos un problema con el archivo de configuraci�n de PHP ( php.ini ). La directiva <em>register_globals</em> est� desactivada, y as� no podemos seguir. 
			Necesitas activarla en el archivo de configuraci�n. Este se encuentra en el directorio /etc/. Ed�talo, busca el texto  
...
<label for="clave"><h5><small>Contrase�a</small></h5></label><input type="password" name="clave" class="input-block-level"  />
<br /><br />
...
mismo nombre de usuario que utilizas para entrar en S�neca) y tu DNI
como clave de acceso. Pasar�s a una p�gina en la que deber�s introducir
una nueva Clave de Acceso, al modo de S�NECA, con la que entrar�s a
partir de entonces. <em style="color: #08c;">Es muy recomendable que
utilices tu clave de S�NECA tambi�n en la Intranet, para simplificarte
la vida y no multiplicar las contrase�as</em>.</p>
<p> Por motivos de
...
<p>Si has olvidado la contrase�a, ponte en contacto con alguien de la Direcci�n del Centro. Se escribir� de nuevo tu DNI como contrase�a y podr�s
crear una nueva como si fuera la primera vez. Para cualquier otro tipo
de problema, ponte tambi�n en contacto. 
</p>
</div>
</div>
<br />
