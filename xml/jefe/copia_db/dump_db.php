...
<?php 
	...
	if( !@function_exists( 'gzopen' ) ) {
		$hay_Zlib = false;
		echo( "- Ya que no está disponible Zlib, guardaré la Base de Datos sin comprimir, como '$filename'<br>" );
	}
	else {
		$filename = $filename . ".gz";
		$hay_Zlib = true;
		echo( "- Ya que está disponible Zlib, guardaré la Base de Datos comprimida, como '$filename'<br>" );
	}
	...
	if( !$error ) { 
		...
		if( !$result ) {
			...
			if( !$filehandle ) {
				...
				setlocale( LC_TIME,"spanish" );
				$tabledump = utf8_decode("-- Fécha: ") . strftime( "%A %d %B %Y - %H:%M:%S", time() ) . "\n";
				if( !$hay_Zlib ) 
					fwrite( $filehandle, $tabledump );
				else
					gzwrite( $filehandle, $tabledump );	
				...
				echo( "<br>" );
				echo( "- La copia de seguridad de la Base de Datos está completo.<br>" );
				echo( "- He guardado la Base de Datos en: $el_path/$filename<br>" );
				echo( "<br>" );
				echo( "- Puedes bajártela directamente: <a href=\"$filename\">$filename</a>" );
