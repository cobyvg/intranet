			...
			<table class='table table-bordered table-condensed'
				...
				<?php } ?>	
					...
					if ($num_ev3>0) {
						echo "<td nowrap>
									<div style='width:40px;height:104px;'>
									<div class='Rotate-90'><span style='font-weight:bold'>Media Final Séneca</span></div>
									</div> </td>";
					}
						...
						while($row = mysqli_fetch_array($result))
						{
							...
							if ($n_fila2=="10" or $n_fila2=="20" or $n_fila2=="30" or $n_fila2=="40") {
								...
								if ($num_ev3>0) {
									echo "<td nowrap>
									<div style='width:40px;height:90px;'>
									<div class='Rotate-corto'>Media Séneca</div>
									</div> </td>";
								}
...
<div class='Rotate-corto'></div>
					...
					<?php
					if ($i==3) {
						$med = substr(($califica_seneca1 + $califica_seneca2 + $califica_seneca3) / 3,0,4);
					?>
					<td style="background-color:#999;color:#fff;vertical-align: middle; text-align: center; height: 74px !important;"><span class=""><?php echo $med; ?></span>
					<?php
							}	
						}
					}
