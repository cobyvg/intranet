...
$enf=mysqli_query($db_con,"show columns from matriculas WHERE field='enfermedad'");
...
$enf=mysqli_query($db_con,"show columns from matriculas WHERE field='foto'");
if (mysqli_num_rows($enf)>0) {}else{
...
exit();
	...
	elseif (mysqli_num_rows($ya_alma) > 0){
		$alma = mysqli_query($db_con, "select apellidos, nombre, provinciaresidencia, fecha, domicilio, localidad, dni, padre, dnitutor, concat(PRIMERAPELLIDOTUTOR2,' ',SEGUNDOAPELLIDOTUTOR2,', ',NOMBRETUTOR2), dnitutor2, telefono, telefonourgencia, correo, concat(PRIMERAPELLIDOTUTOR,' ',SEGUNDOAPELLIDOTUTOR,', ',NOMBRETUTOR), curso, sexo, nacionalidad, matriculas, claveal, unidad, idcurso from alma, unidades where nomunidad=unidad  and (". $conditio1 .")");
		if (mysqli_num_rows($alma) > 0) {
			$al_alma = mysqli_fetch_array($alma);

			if ($al_alma[21]=="101140"){$curso="1ESO";}
			if ($al_alma[21]=="101141"){$curso="2ESO";}
			if ($al_alma[21]=="101142"){$curso="3ESO";}
			...
			$n_curso_ya = $al_alma[15]; $sexo = $al_alma[16]; $nacionalidad = $al_alma[17]; $letra_grupo = substr($al_alma[15],0,-1); $claveal= $al_alma[19];
...
<table align="center" class="table table-bordered">
	...
	?>
		...
		<tr>
			...
			<?php $num1 += 1; ?>
			<div class="form-group">
			<div class="col-sm-2"><select class="form-control" id="optativa<?php echo $num1;?>" name="optativa<?php echo $num1;?><?php echo (isset($opt_rep) && $opt_rep == 1) ? 'has-error"' : '';?>">
				<option value=""></option>
				<?php for ($z = 1; $z < 8; $z++): ?>
		...
		<tr>
		<?php for ($i = 1; $i < 5; $i++): ?>
			<td><!-- ASIGNATURAS DE MODALIDAD -->
			<p class="form-control-static"><?php echo ${it4.$i}[1]; ?></p>
			<p class="form-control-static"><?php echo ${it4.$i}[2]; ?></p>

			<!-- MATEMATICAS --> <?php if($i == 3): ?>
			<div class="form-group">
			<div class="radio"><label> <input type="radio"
				class="itinerario<?php echo $i; ?>" name="matematicas4" value="A"
				<?php echo ($matematicas4 == 'A') ? 'checked' : '' ; ?>
				<?php echo ($itinerario != $i) ? 'disabled' : ''; ?>> <?php echo ${it4.$i}[3]; ?>
			</label></div>
			</div>
			<div class="form-group">
			<div class="radio"><label> <input type="radio"
				class="itinerario<?php echo $i; ?>" name="matematicas4" value="B"
				<?php echo ($matematicas4 == 'B') ? 'checked' : '' ; ?>
				<?php echo ($itinerario != $i) ? 'disabled' : ''; ?>> <?php echo ${it4.$i}[4]; ?>
			</label></div>
			</div>
			<?php else: ?>
		...
		<?php for($i = 1; $i < 5; $i++): ?>
			...
			<div class="form-horizontal"><?php $num1 = ""; ?> <?php $num_it = count(${opt4.$i}); ?>
			<?php foreach(${opt4.$i} as $optit_1 => $nombre): ?> <?php $num1 += 1; ?>
			...
			<label for="<?php echo $optit_1; ?>" class="col-sm-8 control-label">
		...
		<tr>
			...
			<?php $num1 += 1; ?>
			<div
				class="form-group <?php echo (isset($opt_rep2) && $opt_rep2 == 1) ? 'has-error"' : '' ; ?>">
			<div class="col-sm-2"><select class="form-control"
		...
		<div
			...
			<label for="otraenfermedad">Otras enfermedades</label> <input type="text"
