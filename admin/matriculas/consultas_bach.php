...
<table class="table table-striped table-condensed" align="center"
	...
	while($datos_ya = mysql_fetch_object($cons)){
		...
		if (mysql_num_rows($back)>0) {
			$respaldo = '1';
			$backup="<a href='consultas_bach.php?copia=1&id=$id&curso=$curso&consulta=1'><i class='fa fa-refresh' data-toggle='tooltip' title='Restaurar datos originales de la matrícula del alumno '> </i></a>";
		}
...
$text_contr="";
		...
		echo "&nbsp;<a href='consultas_bach.php?borrar=1&id=$id&curso=$curso&consulta=1'><i class='fa fa-trash-o' data-toggle='tooltip' title='Eliminar alumno de la tabla' onClick='return confirmacion();'> </i></a>";
		echo "</td>";
