...
?>
    ...
    <td>
    	<? if($autoriz=="1"){ ?>
    	<span class="fa fa-check-square-o fa-fw fa-lg" data-toggle="tooltip" title="Autorizado"></span>
    	<? }else{ ?> 
    	<span class="fa fa-square-o fa-fw fa-lg" data-toggle="tooltip" title="No autorizado"></span>
    	<? } ?>
    </td>
    <td nowrap><a href="indexextra.php?id=<? echo $datos[0];?>&detalles=1" data-toggle="tooltip" title="Detalles"><span class="fa fa-search fa-fw fa-lg"></span></a>
      <?
	if((stristr($_SESSION['cargo'],'1') == TRUE OR stristr($_SESSION['cargo'],'5') == TRUE)){
?>
  
   <? if($autoriz=="1"){}else{ ?> 
    <a href="indexextra.php?id=<? echo $datos[0];?>&confirmado=1" data-toggle="tooltip" title="Autorizar"><span class="fa fa-check-circle fa-fw fa-lg"></span></a>
    <? } ?>
			...
			if(strlen($id[0]) == 0){echo " <a href='indexextra.php?id=$datos[0]&calendario=1' data-toggle='tooltip' title='Añadir al calendario'><span class='fa fa-calendar fa-fw fa-lg'></span></a>";}
			// hay datos en el Calendario pero la actividad no ha sido registrada.	
			if(strlen($id[0]) > 0 and ($si == "0")){echo " <a href='indexextra.php?id=$datos[0]&act_calendario=1' data-toggle='tooltip' title='Actualizar calendario'><span class='fa fa-refresh fa-fw fa-lg'></span></a>";}?>
			
			<?
			// echo  $_SESSION['dpt']." == ".$datos[4];
			if(stristr($_SESSION['cargo'],'1') == TRUE OR stristr($_SESSION['cargo'],'5') == TRUE  OR (stristr($_SESSION['cargo'],'4') == TRUE and $_SESSION['dpt'] == $datos[4])){
			?>  
			<a href="indexextra.php?id=<? echo $datos[0];?>&eliminar=1" data-toggle="tooltip" title="Eliminar" data-bb="confirm-delete"><span class="fa fa-trash-o fa-fw fa-lg"></span></a>
			<? } ?>
