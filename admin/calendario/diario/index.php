...
if (isset($_GET['id'])) {
	$id =$_GET['id'];
	$dia_cal = mysqli_query($db_con, ("select * from diario where id = '$id'"));
	$diar = mysqli_fetch_array($dia_cal);
	$fecha_reg = $diar[1];
	$grupo = $diar[2];
...
<fieldset><legend>Registrar nueva actividad</legend> <input
...
<div class="input-group"><input type="text" class="form-control"
	id="fecha_reg" name="fecha_reg"
	value="<?php echo (isset($fecha_reg)) ? $fecha_reg : date('Y-m-d'); ?>"
	data-date-format="YYYY-MM-DD" required> <span class="input-group-addon"><i
...
<div class="col-sm-6"><?php $actividades = array("Examen escrito", "Examen oral", "Actividad", "Revisión de actividad", "Otras pruebas", "Anotación personal"); ?>
<div class="form-group"><label for="tipo">Tipo de actividad</label> <select
	class="form-control" id="tipo" name="tipo" required>
	<?php foreach ($actividades as $actividad): ?>
	<option value="<?php echo $actividad; ?>"
	<?php echo (isset($tipo) && $tipo == $actividad) ? 'selected' : ''; ?>><?php echo $actividad; ?></option>
	<?php endforeach; ?>
</select></div>
...
<div class="form-group"><label for="grupos">Unidad (Asignatura)</label>
	...
	id="grupos" name="grupos[]" size="7" multiple>
	<?php while ($row = mysqli_fetch_array($result)): ?>
	<option value="<?php echo $row['grupo'].' => '.$row['materia']; ?>"
	<?php echo (isset($grupos) && in_array($row['grupo'].' => '.$row['materia'], $grupos)) ? 'selected' : ''; ?>><?php echo $row['grupo'].' ('.$row['materia'].')'; ?></option>
	<?php endwhile; ?>
</select> <?php else: ?> <select class="form-control" id="grupos"
	name="grupos[]" size="7" disabled>
	<option value=""></option>
</select> <?php endif; ?></div>

<div class="form-group"><label for="titulo">Título de la actividad</label>
<input type="text" class="form-control" id="titulo" name="titulo"
	value="<?php echo (isset($titulo)) ? $titulo : ''; ?>"
	placeholder="Título de la actividad" required></div>

<div class="form-group"><label for="observaciones">Observaciones</label>
<textarea class="form-control" id="observaciones" name="observaciones"
	rows="5"><?php echo (isset($observaciones)) ? $observaciones : ''; ?></textarea>
</div>

<div class="form-group">
<div class="checkbox"><label> <input type="checkbox" id="calendario"
	name="calendario" value="1"
	<?php echo ((isset($calendario) && $calendario) || !isset($id)) ? 'checked' : ''; ?>>
Mostrar en el calendario </label></div>
</div>

<button type="submit" class="btn btn-primary" name="enviar">Registrar</button>
<button type="reset" class="btn btn-default">Cancelar</button>
	<?php if (isset($id)): ?> <a href="index.php?<? echo $extra;?>"
	class="btn btn-default">Nueva actividad</a> <?php endif; ?></fieldset>
...
<div class="col-sm-5"><?php include("calendario.php"); ?></div>
<!-- /.col-sm-5 --></div>
<!-- /.row -->
...
<table class="table table-striped">
	...
	<?php while ($row = mysqli_fetch_array($result)): ?>
		<tr>
			...
			<td nowrap><a
				href="index.php?id=<?php echo $row['id']; echo "".$extra;?>"><span
				class="fa fa-edit fa-fw fa-lg" data-bs="tooltip" title="Editar"></span></a>&nbsp;
			<a
				href="index.php?id=<?php echo $row['id']; echo "".$extra;?>&borrar=1"
