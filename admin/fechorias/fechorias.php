...
mysql_query("create table if not exists Fechcaduca select id, fecha, TO_DAYS(now()) - TO_DAYS(fecha) as dias from Fechoria");
 ...
 <table class='table table-bordered' style='width:auto' align='center'><tr><td class='expulsion-centro'>Expulsión del Centro</td><td class='amonestacion-escrita'>Amonestación escrita</td><td class='expulsion-aula'>Expulsión del aula</td><td class='aula-convivencia-jefatura'>Aula de convivencia (Jefatura)</td><td class='aula-convivencia-profesor'>Aula de convivencia (Profesor)</td></tr></table><br />";
   ...
   while($row = mysql_fetch_array($result))
		...
		<td  nowrap>"; 
if($_SESSION['profi']==$row[6] or stristr($_SESSION['cargo'],'1') == TRUE){echo "<a href='fechorias.php?id=$id&borrar=1' data-bb='confirm-delete'><i class='fa fa-trash-o fa-fw fa-lg' data-toggle='tooltip' title='Eliminar'></i></a><A HREF='infechoria.php?id=$id&claveal=$claveal'><i class='fa fa-pencil fa-fw fa-lg' data-toggle='tooltip' title='Editar'></i></A>";}	
		echo "<a href='lfechorias2.php?clave=$claveal'><span class='fa fa-user fa-fw fa-lg' data-toggle='tooltip' title='Historial del alumno'></span></a>
		 <A HREF='detfechorias.php?id=$id&claveal=$claveal'><i class='fa fa-search fa-fw fa-lg' data-toggle='tooltip' title='Detalles del problema e historial de problemas del alumno'></i></A></td>
		<td>";
