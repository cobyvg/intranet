...
$query = "SELECT DISTINCT unidad FROM alma WHERE combasi LIKE '%25204%' OR combasi LIKE '%25226%'";
...
while ($unidad = mysqli_fetch_array($unidades)) {
	...
	$result = mysqli_query($db_con, "SELECT nc, CONCAT(apellidos,', ',nombre) AS alumno, claveal FROM FALUMNOS WHERE unidad='$unidad' and claveal in (select claveal from alma where combasi LIKE '%25204%' OR combasi LIKE '%25226%') ORDER BY nc ASC");
	
	$i=0;
	while ($alumno = mysqli_fetch_array($result)) {
		//$div = mysqli_query($db_con,"select combasi from alma where combasi LIKE '%25204%' OR combasi LIKE '%25226%' and claveal");
	...
	foreach($horas as $hora => $nombre) {
		...
		for($i=1;$i<6;$i++) {
			$result = mysqli_query($db_con, "SELECT DISTINCT a_asig, asig FROM horw WHERE a_grupo='$uni_2' AND dia='$i' AND hora='$hora' ");
