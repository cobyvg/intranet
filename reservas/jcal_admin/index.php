...
if (isset($_GET['month'])) { $month = $_GET['month']; $month = preg_replace ("[[:space:]]", "", $month); $month = ereg_replace ("[[:punct:]]", "", $month); $month = ereg_replace ("[[:alpha:]]", "", $month); }
...
echo "<input type=\"hidden\" value=\"$year\" name=\"year\">
      ...
      <input type=\"hidden\" value=\"$today\" name=\"today\">
       <hr><center>
      <input type=\"submit\" id='formsubmit' value='Enviar datos' class='btn btn-primary'> </center>
    </form>";
echo "</div>";
?>
</div>
...
for ($zz = 1; $zz <= $numdays; $zz++) {
  ...
  if ($zz == $today) { 
	...
	.$_SERVER['PHP_SELF']. "?servicio=$servicio&year=$year&today=$zz&month=$month';\" style='background-color:#08c;color:#fff;cursor:pointer;'>$zz</td>";
    $result_found = 1;
  }
...
echo "<td onClick=\"window.location='" .$_SERVER['PHP_SELF']. "?servicio=$servicio&year=$year&today=$zz&month=$month';\" style='background-color:#f89406;color:#fff;cursor:pointer;'>$zz</td>";
        $result_found = 1;
      }
    }
  }
