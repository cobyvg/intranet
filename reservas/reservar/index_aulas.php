...
$alldays = array('Lun','Mar','Mi·','Jue','Vie','S·b','Dom');
$next_year = $year + 1;
$last_year = $year - 1;
    if ($daylong == "Sunday")
	{$daylong = "Domingo";}
...
$reservado1 = mysqli_fetch_row($reservado0);}
	...
	if(!(empty($reservado1[0])) or !(empty($reservado11[0]))) {echo "<label>1™ hora</label> &nbsp;&nbsp; <p class=\"help-block\">Asignada por horario</p>"; }
	else
	{
if ($event_event1  == "") { echo "<label>1™ hora</label> &nbsp;&nbsp; <select name=\"day_event1\" class='form-control'><option></option>";
	$result1 = mysqli_query($db_con, $SQL);
	while($row1 = mysqli_fetch_array($result1)){ $profesor = $row1[0];
	echo "<option>" . $profesor . "</option>";
	} echo "</select>";} 
	else {
	if(mb_strtolower($pr) == mb_strtolower($event_event1)) {echo "<label>1™ hora</label> &nbsp;&nbsp; <input class='form-control' type='text' name=\"day_event1\"  value=\"$event_event1\">";
	 } 
	else{
		echo "<label>1™ hora</label> &nbsp;&nbsp; <input disabled class='form-control' type='text'  value='$event_event1'><input type=\"hidden\" value=\"$event_event1\" name=\"day_event1\">"; }
	}
	}
	echo '</div>';
...
$reservado1 = mysqli_fetch_row($reservado0);}
	echo '<div class="form-group">';
	if(!(empty($reservado1[0])) or !(empty($reservado11[0]))) {echo "<label>2™ hora</label> &nbsp;&nbsp; <p class=\"help-block\">Asignada por horario</p>"; }
	else
	{
if ($event_event2  == "") { echo "<label>2™ hora</label> &nbsp;&nbsp; <select name=\"day_event2\" class='form-control'><option></option>";
	$result1 = mysqli_query($db_con, $SQL);
	while($row1 = mysqli_fetch_array($result1)){ $profesor = $row1[0];
	echo "<option  >" . $profesor . "</option>";
	} echo "</select>";} 
	else {
	if(mb_strtolower($pr) == mb_strtolower($event_event2)) {echo "<label>2™ hora</label> &nbsp;&nbsp; <input class='form-control' type='text' name=\"day_event2\"  value=\"$event_event2\">"; } 
	else{echo "<label>2™ hora</label> &nbsp;&nbsp; <input disabled class='form-control' type='text'  value='$event_event2'><input type=\"hidden\" value=\"$event_event2\" name=\"day_event2\">"; }}	}
	echo '</div>';
...
$reservado1 = mysqli_fetch_row($reservado0);}
	echo '<div class="form-group">';
	if(!(empty($reservado1[0])) or !(empty($reservado11[0]))) {echo "<label>3™ hora</label> &nbsp;&nbsp; <p class=\"help-block\">Asignada por horario</p>"; }
	else
	{
if(($event_event3 == "")) { echo "<label>3™ hora</label> &nbsp;&nbsp; <select name=\"day_event3\" class='form-control'><option></option>";
	$result1 = mysqli_query($db_con, $SQL);
	while($row1 = mysqli_fetch_array($result1)){ $profesor = $row1[0];
	echo "<option  >" . $profesor . "</option>";
	} echo "</select>";}
	else {
	if(mb_strtolower($pr) == mb_strtolower($event_event3)) {echo "<label>3™ hora</label> &nbsp;&nbsp; <input class='form-control' type='text' name=\"day_event3\"  value=\"$event_event3\">"; } 
	else{echo "<label>3™ hora</label> &nbsp;&nbsp; <input disabled class='form-control' type='text'  value='$event_event3'><input type=\"hidden\" value=\"$event_event3\" name=\"day_event3\">"; }}	}	
	echo '</div>';
...
$reservado1 = mysqli_fetch_row($reservado0);}
	echo '<div class="form-group">';
	if(!(empty($reservado1[0])) or !(empty($reservado11[0]))) {echo "<label>4™ hora</label> &nbsp;&nbsp; <p class=\"help-block\">Asignada por horario</p>"; }
	else
	{
if ($event_event4  == "") { echo "<label>4™ hora</label> &nbsp;&nbsp; <select name=\"day_event4\" class='form-control'><option></option>";
	$result1 = mysqli_query($db_con, $SQL);
	while($row1 = mysqli_fetch_array($result1)){ $profesor = $row1[0];
	echo "<option  >" . $profesor . "</option>";
	} echo "</select>";} 
	else {
	if(mb_strtolower($pr) == mb_strtolower($event_event4)) {echo "<label>4™ hora</label> &nbsp;&nbsp; <input class='form-control' type='text' name=\"day_event4\"  value=\"$event_event4\">"; } 
	else{echo "<label>4™ hora</label> &nbsp;&nbsp; <input disabled class='form-control' type='text'  value='$event_event4'><input type=\"hidden\" value=\"$event_event4\" name=\"day_event4\">"; } }	}	
	echo '</div>';
...
$reservado1 = mysqli_fetch_row($reservado0);}
	echo '<div class="form-group">';
	if(!(empty($reservado1[0])) or !(empty($reservado11[0]))) {echo "<label>5™ hora</label> &nbsp;&nbsp; <p class=\"help-block\">Asignada por horario</p>"; }
	else
	{
if ($event_event5  == "") { echo "<label>5™ hora</label> &nbsp;&nbsp; <select name=\"day_event5\" class='form-control'><option></option>";
	$result1 = mysqli_query($db_con, $SQL);
	while($row1 = mysqli_fetch_array($result1)){ $profesor = $row1[0];
	echo "<option  >" . $profesor . "</option>";
	} echo "</select>";} 
	else {
	if(mb_strtolower($pr) == mb_strtolower($event_event5)) {echo "<label>5™ hora</label> &nbsp;&nbsp; <input class='form-control' type='text' name=\"day_event5\"  value=\"$event_event5\">"; } 
	else{echo "<label>5™ hora</label> &nbsp;&nbsp; <input disabled class='form-control' type='text'  value='$event_event5'><input type=\"hidden\" value=\"$event_event5\" name=\"day_event5\">"; }}	}	
	echo '</div>';
...
$reservado1 = mysqli_fetch_row($reservado0);}
	echo '<div class="form-group">';
	if(!(empty($reservado1[0])) or !(empty($reservado11[0]))) {echo "<label>6™ hora</label> &nbsp;&nbsp; <p class=\"help-block\">Asignada por horario</p>"; }
	else
	{
if ($event_event6  == "") { echo "<label>6™ hora</label> &nbsp;&nbsp; <select name=\"day_event6\" class='form-control'><option></option>";
	$result1 = mysqli_query($db_con, $SQL);
	while($row1 = mysqli_fetch_array($result1)){ $profesor = $row1[0];
	echo "<option  >" . $profesor . "</option>";
	} echo "</select>";} 
	else {
	if(mb_strtolower($pr) == mb_strtolower($event_event6)) {echo "<label>6™ hora</label> &nbsp;&nbsp; <input class='form-control' type='text' name=\"day_event6\"  value=\"$event_event6\">"; } 
	else{echo "<label>6™ hora</label> &nbsp;&nbsp; <input disabled class='form-control' type='text'  value='$event_event6'><input type=\"hidden\" value=\"$event_event6\" name=\"day_event6\">"; } }	}	
	echo '</div>';
...
$reservado1 = mysqli_fetch_row($reservado0);}
	echo '<div class="form-group">';
	if(!(empty($reservado1[0]))) {echo "<label>7™ hora</label> &nbsp;&nbsp; <p class=\"help-block\">Asignada por horario</p>"; }
	else
	{
if ($event_event7  == "") { echo "<label>7™ hora</label> &nbsp;&nbsp; <select name=\"day_event7\" class='form-control'><option></option>";
	$result1 = mysqli_query($db_con, $SQL);
	while($row1 = mysqli_fetch_array($result1)){ $profesor = $row1[0];
	echo "<option  >" . $profesor . "</option>";
	} echo "</select>";} 
	else {
	if(mb_strtolower($pr) == mb_strtolower($event_event7)) {echo "<label>7™ hora</label> &nbsp;&nbsp; <input class='form-control' type='text' name=\"day_event7\"  value=\"$event_event7\">"; } 
	else{echo "<label>7™ hora</label> &nbsp;&nbsp; <input disabled class='form-control' type='text'  value='$event_event7'><input type=\"hidden\" value=\"$event_event7\" name=\"day_event7\">"; }}
	echo '</div>';
	}
...
<p class="text-justify">Dadas las necesidades de espacios educativos que nuestro instituto ha ido teniendo en los ˙ltimos cursos, el uso del Aula Magna se fue incrementando y esa masificaciÛn e indiscriminaciÛn en su utilizaciÛn derivÛ en algunas situaciones que han hecho replantearnos algunos de los protocolos que tenÌamos para un espacio tan fr·gil como es el Aula Magna. De hecho, el centro ha tenido que hacer frente a varias facturas de diferentes empresas que han tenido que reparar algunos de sus elementos: persianas, sonido, iluminaciÛn, albaÒilerÌa... </p>

<p class="text-justify">Con la intenciÛn de reconducir la situaciÛn, publicamos una serie de normas de cara a su posible utilizaciÛn por parte de miembros de la comunidad educativa del centro. Son, intentando recrear un orden cronolÛgico, las siguientes:</p>

<ul>
<li class="text-justify">Reserva del Aula Magna. Si el uso es para la maÒana ya no se puede llevar a cabo a trav·s del mÛdulo que hay en la Intranet puesto que se ha deshabilitado esa opciÛn. Este tipo de reserva Äúmatutina solo se puede hacer hablando con el alguien del equipo directivo o la persona responsable del D.A.C.E., salvo las ya asignadas por horario del centro desde el inicio de curso (Taller de Teatro, por ejemplo). Si la utilizaciÛn es para casos excepcionales (ex·menes de varios grupos...) para la tarde, sÌ que se debe hacer usando el mÛdulo de la Intranet, marcando la 7™ hora.  </li> 

<li class="text-justify">Si la reserva del Aula Magna implica la necesidad del uso de medios audiovisuales ( llave de la mesa de sonido, la mesa de sonido, megafonÌa, pantalla, ordenador, videoproyector, mando de aire acondicionado/calefacciÛn...), se deber· contactar con M™. Carmen Gal·n, del Departamento de Actividades Complementarias y Extraescolares, o Paco PÈrez, vicedirector, puesto que su custodia y mantenimiento solo lo llevan ellos. Es por eso que la devoluciÛn del citado material, en el mismo estado en el que fue entregado, se har· a uno de ellos dos.</li>

<li class="text-justify">Cuando se coja la llave de ConserjerÌa para acceder al Aula Magna, adem·s de apuntarse en el listado correspondiente se tendr· la precauciÛn de abrir ambas puertas (la misma llave abre las dos puertas verdes acristaladas) y la doble hoja, aunque haya que dejarlas entornadas por motivos de necesidad de una opacidad en el Aula. Esto es para evitar problemas en el caso de evacuaciÛn por emergencia. No estamos hablando de la llave ni de la puerta de acceso al p·rking de profesores. Esa llave tambi·n se encuentra en el juego de llaves del Aula Magna en la ConserjerÌa. </li>

<li class="text-justify">Comprobar si la alarma est· o no activada. HabrÌa que preguntar antes la clave de la alarma si no supiera con anterioridad. Aparece un mensaje en la pantallita de la alarma que dice: "ÄúSistema Activo"Äù.  </li>

<li class="text-justify">En el caso de que haya que subir las persianas, abrir las ventanas y/o descorrer las cortinas rojas, se encargar· de ello ˙nica y exclusivamente el profesor responsable de la actividad para evitar malos usos. Del mismo modo se proceder· cuando toque bajar las persianas, cerrar las ventanas y/o correr las cortinas.</li>

<li class="text-justify">Se deber· mantener la limpieza en el Aula Magna de forma que el alumnado utilice las papeleras que hay en la entrada y en los servicios. Asimismo, se intentar· mantener el orden y la disposiciÛn de las sillas-palas para futuros usos. No es de recibo que uno vaya al Aula Magna a una conferencia con el alcalde de Estepona, por ejemplo, y el Aula Magna est· sucia y/o desordenada.</li>

<li class="text-justify">Comprobar antes de dejar el Aula Magna que todo est· como debiera estar (luces apagadas, persianas echadas, ventanas cerradas...) y, antes de salir, poner la alarma si es necesario. </li>
</ul>

<p class="text-justify">Agradecemos de antemano la predisposiciÛn de todos para con el cuidado de este importante espacio educativo de nuestro instituto. Tan ˙nico como valioso!</p>
