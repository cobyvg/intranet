...
<h4 class="modal-title" id="myModalLabel">InformaciÛn sobre la reserva de aulas.</h4>
</div>
<div class="modal-body">
<p class="help-block text-info"><small class="text-info">
El Equipo directivo puede asignar el horario de las dependencias del Centro de varias maneras. Si el aula aparece en el Horario y este se ha importado en la base de datos, se presentar·n las horas del aula con la asignatura que se imparte en ese momento con la expresiÛn <strong>Asignada por Horario</strong>. El Equipo directivo puede asignar el aula a otro profesor por encima del Horario si por alguna razÛn lo considera necesario. La hora aparecer· entonces en el formulario con la expresiÛn <strong>Asignada por DirecciÛn.</strong><br>
Si el aula est· vacÌa en ese momento cualquier profesor puede seleccionar esa hora para utilizar el aula.<br>
Si queremos asignar una hora a un profesor durante todo el curso escolar, seleccionamos al mismo en el formulario y pulsamos sobre el botÛn rojo <strong>Reservar todo el Curso</strong>. Es importante elegir un dÌa sin reserva alguna de profesores para realizar la asignaciÛn permanente porque ls profesores que aparezcan en el formulario quedar·n fijados para todo el curso escolar. Los campos en los que aparezca la palabra "AsignaciÛn" (bien por Horario o bien por DirecciÛn) no se registran en la reserva permanete. A partir de ese momento los profesores ver·n en ese campo la expresiÛn <strong>Asignada por DirecciÛn</strong></strong> y no podr·n elegir el aula en esa hora. Este procedimiento se puede aplicar tantas veces como queramos para aquellas dependencias o aulas que aparezcan en el horario.   
</small></p>
</div>
...
if($aula){
	...
	for ($i = 1; $i < 8; $i++)
	{
		...
		$aula_hor = "SELECT hora$i FROM `".$aula."hor` WHERE dia = '$numero_dia'";
		...
		if(stristr($_SESSION['cargo'],'1') == TRUE)
		{
			echo "<label>".$i."™ hora</label> &nbsp;&nbsp; <select name=\"day_event$i\" class='form-control'>";

			if(strlen(${event_event.$i})>0){
				echo "<option>${event_event.$i}</option>";
		}
		elseif (strlen($num_aula_hor[0])>0) {
			echo "<option>Asignada por DirecciÛn: $num_aula_hor[0]</option>";
		}
		...
		if (strlen($num_hor[0])>0) {
			echo "<label>".$i."™ hora</label> &nbsp;&nbsp; <p class=\"help-block text-info\">Asignada por horario</p>";
		}
		elseif (strlen($num_aula_hor[0])>0) {
			echo "<label>".$i."™ hora</label> &nbsp;&nbsp; <p class=\"help-block text-danger\">Asignada por DirecciÛn</p>";
		}
		else
		{
			if (${event_event.$i}  == "") {
				...
				if(mb_strtolower($pr) == mb_strtolower(${event_event.$i})) {
					echo "<label>".$i."™ hora</label> &nbsp;&nbsp; <input class='form-control' type='text' name=\"day_event$i\"  value=\"${event_event.$i}\">";
			}
				else{
				echo "<label>".$i."™ hora</label> &nbsp;&nbsp; <input disabled class='form-control' type='text'  value='${event_event.$i}'><input type=\"hidden\" value=\"${event_event.$i}\" name=\"day_event$i\">";
	}
}
	}
	}
...
<ul>
	...
	maÒana ya no se puede llevar a cabo a trav·s del mÛdulo que hay en la
	...
	puerta de acceso al p·rking de profesores. Esa llave tambi·n se
