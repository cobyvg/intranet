...
// Actualizamos estructura del Calendario y lo preparamos para recoger todas las actividades extraescolares
$actua = mysqli_query($db_con, "select modulo from actualizacion where modulo = 'Estructura calendario'");
if (mysqli_num_rows($actua)>0) {}else{
	mysqli_query($db_con,"ALTER TABLE `calendario` ADD `confirmado` TINYINT(1) NOT NULL , ADD `observaciones` TEXT NULL");
	mysqli_query($db_con,"ALTER TABLE `calendario` CHANGE `nombre` `nombre` VARCHAR(255) CHARACTER SET latin1 COLLATE latin1_spanish_ci NOT NULL");
	mysqli_query($db_con, "insert into actualizacion (modulo, fecha) values ('Estructura calendario', NOW())");
	mysqli_query($db_con,"update calendario set confirmado = '1' where categoria = '2'");
	
	$act = mysqli_query($db_con,"select id, actividad, confirmado from actividades");
	while ($activ = mysqli_fetch_array($act)) {
		$cl = mysqli_query($db_con,"select id from calendario where nombre like '$activ[1]' and categoria = '2'");
		$cld = mysqli_fetch_array($cl);
		if (mysqli_num_rows($cl)>0) {
			$res = mysqli_query($db_con, "SELECT id FROM actividades WHERE id like '$activ[0]'");
			$row_aa = mysqli_fetch_array($res);
			mysqli_query($db_con,"update actividadalumno set cod_actividad = '$cld[0]' where cod_actividad = '$row_aa[0]'");			
		}
		else{
			$result = mysqli_query($db_con, "SELECT actividad, descripcion, departamento, profesor, grupos, fecha, hoy, confirmado, id FROM actividades WHERE id='$activ[0]'");
			...
			$confirmado = mysqli_real_escape_string($db_con, $row['confirmado']);
			$query = "INSERT INTO `calendario` (`categoria`, `nombre`, `descripcion`, `fechaini`, `horaini`, `fechafin`, `horafin`, `departamento`, `profesores`, `unidades`, `fechareg`, `profesorreg`, `confirmado`) VALUES (2, '$nombre', '$descripcion', '".$fechaini."', '08:15', '".$fechaini."', '09:15', '$departamento', '$profesores', '$unidades', '".$fechareg."', 'admin', '$confirmado')";
