...
<div class="well" align="left"><!-- Button trigger modal --> <a href="#"
...
introducir datos sobre los alumnos de un grupo. Las columnas pueden ser
de varios tipos: las numéricas tienen una caja de datos pequeña y solo
admiten números con decimales; las columnas de texto largo son más
grandes y pueden contener caracteres alfanuméricos; etc. <br />
...
$mens1 = "cuaderno.php?profesor=$pr&asignatura=$asignatura&dia=$dia&hora=$hora&curso=$curso_sin&seleccionar=1'";
...
echo "</ul>";
?></div>
<div class="well" align="left" style="min-width: 250px;"><!-- Button trigger modal -->
...
<div class="modal-body">
<p class="help-block">Hay varias funciones que puedes realizar sobre las
columnas que contienen los datos (ocultar, eliminar, calcular medias,
etc). Marca las columnas sobre las que quieres trabajar, y luego
presiona el botón que realiza una determinada operación sobre esas
columnas. No te olvides de seleccionar las columnas correspondientes.</p>
</div>
<div class="modal-footer">
<button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
</div>
...
if (mysqli_num_rows($colum0) > 0) {
	...
	while($colum00=mysqli_fetch_array($colum0)){
		...
		$otra3=mysqli_query($db_con, "select * from notas_cuaderno where profesor = '$pr' and curso = '$curs0' and asignatura='$asignatura' order by orden asc");
		...
		while ($colum1 = mysqli_fetch_array($otra2)) {
			...
			echo "<td>";
			?>
<div class="checkbox"><input type="checkbox" name="<? echo $id;?>"
...
?>
<form action="cuaderno/orden.php<? echo $extra;?>" method="POST"
	id="orden">
...
<td nowrap style='vertical-align: middle' class='text-info'
	style='width:auto;'><a href="" onclick="window.open('<? echo $inf;?>')">
<?
...
<div class="checkbox"><input name="select_<? echo $row[1]."_".$curso;?>"
	type="checkbox" id="selal" <? if ($marcado == "1") {echo "checked ";}?>
	value="1" /></div>
</td>
			<?
		}
