...
if (isset($_POST['submit'])) {
	...
	foreach ($directorio as $archivo) {
	    ...
	    if (!is_dir($archivo) and stristr($archivo,".xml")==TRUE)
	    {
	        ...
	        foreach( $tag_alumno as $alumno ) {
	        	...
	        	foreach( $tag_faltas as $falta ) {
		    		...
		    		if ($L_DIACOM == 'S') {
		    			...
		    			// Obtenemos el código del profesor y de la asignatura que se imparte en el día y hora
			    		$result = mysqli_query($db_con, "SELECT no_prof, c_asig FROM horw WHERE dia = '".strftime("%u", strtotime(fecha_mysql($F_FALASI)))."' AND hora = '$hora_tramo' AND a_grupo = '$T_NOMBRE'");
			    		while ($row = mysqli_fetch_assoc($result)) {
			    			if(in_array($row['c_asig'], $asignaturas)) {
			    				$nprofesor = $row['no_prof'];
			    				$codasig = $row['c_asig'];
	        ...
	        echo '<span class="text-success">Importado</span></li>';
	        $draw_html = 1;
...
?>
		...
		<div class="alert alert-warning">
			Ya existe información en la base de datos. Este proceso sobreescribirá la información de la Intranet. Es recomendable realizar una <a href="copia_db/index.php" class="alert-link">copia de seguridad</a> antes de proceder a la importación de los datos.
		</div>
		<?php endif; ?>
			...
			<div class="col-sm-6">
				...
				<div class="well">
					<form action="" method="POST" enctype="multipart/form-data">
						...
						<fieldset>
							<legend>Importación de faltas de asistencia</legend>

							<div class="form-group">
							  <label for="archivo"><span class="text-info">Exportacion_Faltas_Alumnado.zip</span></label>
